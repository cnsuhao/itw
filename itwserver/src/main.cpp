...
void testOcr() 
{
	// 修改为dll所在目录，这里.表示是当前目录
	const char * libFolder = "../lib";  
	// 修改需要识别的图片目录，
	const char * fileImg = "../image/imaged.png"; 
	...
	//		"PROP_PDF_OUTPUT_FILE=result.pdf,PROP_PDF_OUTPUT_TEXT_VISIBLE=false,PROP_PDF_OUTPUT_IMAGE_FORCE_BW=true,PROP_OUTPUT_SEPARATE_WORDS=false,PROP_PDF_OUTPUT_RETURN_TEXT=text", ",", "=");

	char * s = c_com_asprise_ocr_recognize(ptrToApi, fileImg, -1, -1, -1, -1, -1, OCR_RECOGNIZE_TYPE_ALL, OCR_OUTPUT_FORMAT_XML,
			"PROP_PDF_OUTPUT_RETURN_TEXT=result.xml", ",", "=");
